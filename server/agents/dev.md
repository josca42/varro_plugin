...
This is what MCP clients (Claude Code, Codex) invoke via their config. The plugin's `.mcp.json` launches `bin/varro-mcp`, which resolves the plugin root and then runs this module with `uv --project <plugin-root>/server`. The MCP server does not start the dashboard — the snapshot tool takes a full URL and relies on the dashboard already running.
