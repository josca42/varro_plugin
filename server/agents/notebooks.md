...
The `jupyter` and `sql` tools persist state across conversations by appending each cell to a `.py` file under `<project>/notebooks/`. On a fresh server start (or notebook switch), the file is replayed into a new IPython shell to rebuild kernel state. No pickling, no extra MCP tool.

## Lifecycle

- On boot, `varro/main.py` calls `_switch("sandbox")`, which ensures `<project>/notebooks/sandbox.py` exists and replays it. `current_notebook` defaults to `sandbox.py`.
...
- Notebook files live under `{project_dir}/notebooks`. The MCP project root defaults to cwd; override with `VARRO_PROJECT_DIR`.
